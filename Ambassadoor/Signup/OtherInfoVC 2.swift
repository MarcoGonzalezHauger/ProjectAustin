 ...
 class OtherInfoVC: UIViewController, UITextFieldDelegate, CategoryExplainedDelegate, EnterZipCode {
	 
	 func ZipCodeEntered(zipCode: String?) {
		 if let zipCode = zipCode {
			 zipcode_Txt.text = zipCode
			 GetTownName(zipCode: zipCode) { (TownName, zipCode) in
				 if self.zipcode_Txt.text == zipCode {
					 self.townNameLabel.text = TownName?.CityAndStateName ?? ""
				 }
			 }
		 }
	 }
	 ...
	 func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
		 if textField == primeCat_Txt {
			 ...
			 performSegue(withIdentifier: "toZipSelect", sender: self)
			 //toZipSelect
			 return false
		 } else {
	 ...
	 override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		 ...
		 if let destination = segue.destination as? ZipCodeVC {
			 ...
			 destination.noCancel = zipcode_Txt.text!.isEmpty
		 }
